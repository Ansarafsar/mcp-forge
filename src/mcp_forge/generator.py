...
def _render_tool(tool: ToolSpec, spec: ForgeSpec) -> str:
    ...
    if spec.auth:
        body = "    _forge_auth()\n" + body
    decorators = "@mcp.tool()\n"
    for line in runtime.tool_decorators(spec, tool.name):
        decorators += line + "\n"
    return f"{decorators}def {tool.name}({sig}) -> {returns}:\n{doc}{body}"
...
def generate(spec: ForgeSpec) -> str:
    ...
    return template.render(
        spec=spec,
        prelude=prelude,
        forge_runtime=runtime.build_runtime(spec),
        typing_imports=sorted(_needs_typing(spec)),
        tools=[_render_tool(t, spec) for t in spec.tools],
...
_DOCKERFILE = """\
# Generated by MCP-Forge for the "{name}" server.
...
def docker_files(spec: ForgeSpec) -> dict[str, str]:
    """Return the files for a Docker build target, keyed by filename."""
    return {
        "server.py": generate(spec),
        "requirements.txt": "mcp>=1.2.0\n",
        "Dockerfile": _DOCKERFILE.format(name=spec.name),
